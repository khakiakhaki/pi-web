...
- **Last session restore**: opening the bare home URL restores the last session selected in that browser origin. An explicit `?session=<id>` URL takes priority; different hosts/IPs keep separate browser caches.
- **Model config**: the Models panel reads and writes `models.json` in the pi agent directory. Model lists and defaults come from pi's config.
- **HTTP proxy**: set `PI_PROXY=http://localhost:<port>` (or `pi_proxy`) to route pi-web model calls through that proxy. Leave it unset to use the normal direct path.
